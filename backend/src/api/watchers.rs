...
pub fn router() -> Router<SqlitePool> {
    Router::new()
        // Basic watcher operations
        .route("/", get(get::list_watchers).post(post::create_watcher))
        // Enhanced dashboard endpoints
        .route("/enhanced", get(get::list_watchers_enhanced))
        .route("/{id}/status", get(get::watcher_status_detail))
        .route("/{id}/history", get(get::watcher_sync_history))
        // Watcher-specific operations by name
        .route("/{watchername}/start", get(get::start_watcher))
        .route("/{watchername}/stop", get(get::stop_watcher))
