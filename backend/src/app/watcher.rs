...
use crate::users::repository::{SyncRepository, WatcherRepository};
use anyhow::Result;
use sqlx::SqlitePool;
...
impl Watcher {
    ...
    pub async fn start_watcher_task(&self, watcher_id: i64) {
        ...
        let sync_frequency = match watcher_repo.get_watcher_by_id(watcher_id).await {
            ...
            _ => {
                tracing::warn!(
                    "Could not get watcher {}, using default frequency",
                    watcher_id
                );
        ...
        tracing::info!(
            "Started watcher task {} with {}s interval",
            watcher_id,
            sync_frequency
        );
    }
    ...
    async fn sync_watcher(
        ...
        let sync_op = sync_repo
            .create_sync_operation(watcher_id, "auto_sync")
            .await?;
        ...
        match result {
            ...
            Err(e) => {
                ...
                sync_repo
                    .update_sync_operation(sync_op.id, "failed", 0, 0, 0, Some(&error_msg))
                    .await?;
    ...
    async fn sync_spotify_watcher(
        ...
        let operation_result = sync_service
            .sync_playlist_to_target(watcher_id)
            .await
    ...
    pub async fn preview_sync(
        &self,
        watcher_id: i64,
    ) -> Result<crate::users::models::SyncPreviewResponse, WatcherError> {
        ...
        match watcher.source_service.as_str() {
            ...
            _ => Err(WatcherError::ServiceError(format!(
                "Unsupported source service: {}",
                watcher.source_service
            ))),
        }
    }

    async fn preview_spotify_sync(
        &self,
